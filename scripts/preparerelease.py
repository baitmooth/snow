...
parser.add_argument('SNOW_DIR', type=str, help='Path to the Snow directory')

args = parser.parse_args()

SNOW_DIR = os.path.abspath(args.SNOW_DIR)

def check_snow_path(path):
    # Check if the given path includes "Snow" folder or if it is one level below
    snow_folder = os.path.join(path, "snow")
    if os.path.exists(snow_folder) and os.path.isdir(snow_folder):
        return snow_folder
    else:
...
SNOW_PATH = check_snow_path(SNOW_DIR)

#Define Path
NEWICONS_PATH = SNOW_PATH +"/newicons"
ICONS_PATH = SNOW_PATH +"/icons"
APP_SRC_DIR = SNOW_PATH + "/app/src"
